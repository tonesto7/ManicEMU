...
extension GameType
{
    static let ms = GameType("public.aoshuang.game.ms")
}

@objc enum MSGameInput: Int, Input, CaseIterable {
    case a
    case b
    case start
    case select
...
struct MS: ManicEmuCoreProtocol {
    public static let core = MS()
    
    public var name: String { "MS" }
    public var identifier: String { "com.aoshuang.MSCore" }
    
    public var gameType: GameType { GameType.ms }
    public var gameInputType: Input.Type { MSGameInput.self }
    var allInputs: [Input] { MSGameInput.allCases }
    ...
    public var emulatorConnector: EmulatorBase { MSEmulatorBridge.shared }
...
class MSEmulatorBridge : NSObject, EmulatorBase {
    static let shared = MSEmulatorBridge()
    ...
    func activateInput(_ input: Int, value: Double, playerIndex: Int) {
        ...
        if let gameInput = MSGameInput(rawValue: input),
...
Log.debug("\(String(describing: Self.self))点击了:\(gameInput)")
    ...
    func gameInputToCoreInput(gameInput: MSGameInput) -> LibretroButton? {
