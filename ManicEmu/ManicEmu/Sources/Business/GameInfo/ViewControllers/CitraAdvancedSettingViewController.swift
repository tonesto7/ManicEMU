//
//  CitraAdvancedSettingViewController.swift
//  ManicEmu
//
...
class CitraAdvancedSettingViewController: QuickTableViewController {
    
    enum SettingType {
        case none, `switch`, option, action
    }
    
    private var currentConfig = try? INIFile(fileName: Constants.Path.CitraConfig)
    ...
    private lazy var moreContextMenuButton: ContextMenuButton = {
        ...
        actions.append((UIAction(title: R.string.localizable.controllerMappingReset()) { [weak self] _ in
            ...
            self.currentConfig = try? INIFile(fileName: Constants.Path.CitraConfig)
            self.updateData()
        }))
        actions.append(UIAction(title: R.string.localizable.shareConfigButtonTitle()) { [weak self] _ in
            ...
            ShareManager.shareFile(fileUrl: URL(fileURLWithPath: Constants.Path.CitraConfig))
        })
        actions.append((UIAction(title: R.string.localizable.importConfigButtonTitle()) { [weak self] _ in
            ...
            FilesImporter.shared.presentImportController(supportedTypes: UTType.configTypes, allowsMultipleSelection: false) { [weak self] urls in
                ...
                if let url = urls.first {
                    do {
                        try FileManager.safeCopyItem(at: url, to: URL(fileURLWithPath: Constants.Path.CitraConfig), shouldReplace: true)
                        self.currentConfig = try INIFile(fileName: Constants.Path.CitraConfig)
                        self.updateData()
                    } catch {
